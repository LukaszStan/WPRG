...
?>
<h2>Artykuły stworzone przez <?php echo $autor;?></h2>
<?php
$conn = mysqli_connect("localhost","root","","portal_info");
$zap1 = "SELECT artykulyID,tytul,uzytkownicy.login AS autor,tresc,data,obrazek,dzial FROM artykuly INNER JOIN uzytkownicy ON artykuly.uzytkownicy_uzytkownikID = uzytkownicy.uzytkownikID WHERE uzytkownicy.login = '$autor' ORDER BY data DESC;";
...
while ($row = mysqli_fetch_array($wyn)){
    ...
    echo "
        ...
        Autor: ${row['autor']}, ${row['data']}<br/><br/>
        ";
}
